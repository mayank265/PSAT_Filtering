...
def Acceleration_U():
    for i in range(N):
        if i == 0:
            ...
            data.at[i, "accl_U"] = (data.at[i, "U"] - data.at[i - 1, "U"]) / data.at[
                1, "Time"
            ]
...
def Acceleration_V():
    for i in range(N):
        if i == 0:
            ...
            data.at[i, "accl_V"] = (
                data.at[i, "V"] - data.at[i - 1, "V"] / data.at[1, "Time"]
            )
...
def Acceleration_W():
    for i in range(N):
        if i == 0:
            ...
            data.at[i, "accl_W"] = (
                data.at[i, "W"] - data.at[i - 1, "W"] / data.at[1, "Time"]
            )
...
def replace_all_4(i):
    if i <= 12 or i > N - 12:
        ...
        y2 = i + 13
        data.at[i, "U"] = (
            data["U"].iloc[x1:y1].mean() + data["U"].iloc[x2:y2].mean()
        ) / 2
        data.at[i, "V"] = (
            data["V"].iloc[x1:y1].mean() + data["V"].iloc[x2:y2].mean()
        ) / 2
        data.at[i, "W"] = (
            data["W"].iloc[x1:y1].mean() + data["W"].iloc[x2:y2].mean()
        ) / 2
...
def acceleration_all_at_a_time_negative(x):
    ...
    while found_peak and max_iteration > 0:
        ...
        for i in range(N):
            ...
            if (
                (
                    data.at[i, "accl_U"] < -1 * Lambda * g
                    and data.at[i, "U"] < -1 * data.at[0, "U_std"] * k
                )
                or (
                    data.at[i, "accl_V"] < -1 * Lambda * g
                    and data.at[i, "V"] < -1 * data.at[0, "V_std"] * k
                )
                or (
                    data.at[i, "accl_W"] < -1 * Lambda * g
                    and data.at[i, "W"] < -1 * data.at[0, "W_std"] * k
                )
            ):
                found_peak = 1
                replace_all(i, x)
...
def acceleration_one_at_a_time_negative(x):
    ...
    while found_peak and max_iteration > 0:
        ...
        for i in range(N):
            if i == 0:
                continue
            if ((data.at[i, "accl_U"] * 1) < -1 * Lambda * g) and (
                (data.at[i, "U"] * 1) < (-1 * data.at[0, "U_std"] * k)
            ):
                found_peak = 1
                replace_U(i, x)
            if ((data.at[i, "accl_V"] * 1) < -1 * Lambda * g) and (
                (data.at[i, "V"] * 1) < (-1 * data.at[0, "V_std"] * k)
            ):
                found_peak = 1
                replace_V(i, x)
            if ((data.at[i, "accl_W"] * 1) < -1 * Lambda * g) and (
                (data.at[i, "W"] * 1) < -1 * data.at[0, "W_std"] * k
            ):
                found_peak = 1
                replace_W(i, x)
...
def Corr_All(x, corr):
    for i in range(N):
        if i == 0:
            continue
        if (
            int(data.at[i, "Corr_U"]) < int(corr)
            or int(data.at[i, "Corr_V"]) < int(corr)
            or int(data.at[i, "Corr_W"]) < int(corr)
        ):
            replace_all(i, x)
...
def SNR_All(x, SNR):
    for i in range(N):
        if i == 0:
            continue
        if (
            int(data.at[i, "SNR_U"]) < int(SNR)
            or int(data.at[i, "SNR_V"]) < int(SNR)
            or int(data.at[i, "SNR_W"]) < int(SNR)
        ):
            replace_all(i, x)
